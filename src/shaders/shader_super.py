...
def connect_code(node: 'Node', code: GLSLCode):
    ...
    :param code: The parsed shader other_code that is held by the Node 'node'
    """
    code.reset()
    for _, socket in node.get_in_sockets().items():
        assert socket.socket_type == Socket.SOCKET_INPUT

        socket_arg = socket.argument
...
class Shader(ABC):
    ...
    def _parse(self):
        assert self.__class__.FRAGMENT_SHADER_FILENAME, "Name of GLSL fragment shader need to be set by subclass to FRAGMENT_SHADER_FILENAME " \
                                                        "constant."
        assert self.__class__.FRAGMENT_SHADER_FUNCTION, "Name of GLSL fragment shader main primary_function need to be set by subclass {} to " \
    ...
    def get_inputs(self) -> typing.List[typing.Tuple[str, str, str, typing.Tuple[float, float], Tensor]]:
        """Returns a list of tuples with information about the widgets parameters of this shader.
            ...
            :return: a tuple on the form (formatted name, argument name, internal type, (min value, max value), default value)
        """
    ...
    @abstractmethod
    def shade_mat(self, vert_pos: Tensor, *args) -> Tensor:
        pass


class FunctionShader(Shader, ABC):
    """A shader that is represented by only a single primary_function in GLSL, and can thus on its own not be made into a program."""
    FRAGMENT_SHADER_FILENAME = None
    verified = False
    ...
    def _verify_arguments(self):
        ...
        assert glsl_args[0].name == shade_args[0] and glsl_args[0].name == shade_mat_args[0], \
            "Name of first argument of shader functions (both in python and GLSL) should be vert_pos, not {}".format(glsl_args[0].name)

        for py_arg, shade_arg, shade_mat_arg, glsl_arg in zip(py_args, shade_args[1:], shade_mat_args[1:], glsl_args[1:]):
            assert py_arg == shade_arg, "Python input argument name and argument to python shade primary_function do not coincide: {} != {}" \
                .format(py_arg, shade_arg)
            assert py_arg == shade_mat_arg, "Python input argument name and argument to python matrix shade primary_function do not coincide: {} != {}" \
                .format(py_arg, shade_mat_arg)
            assert py_arg == glsl_arg.name, "Python input argument name and argument to GLSL shade primary_function do not coincide: {} != {}" \
...
class CompilableShader(Shader, ABC):
    ...
    def compile_vertex_shader(self, new_instance=False) -> gloo.VertexShader:
    ...
    def compile_fragment_shader(self, node: 'Node') -> gloo.FragmentShader:
        ...
        :param node: The Node object that represents this shader in the node graph. If this is not provided, this primary_function will only compile the
        other_code belonging to this shader.
        ...
        if not node:
            ...
            uniforms = output_code.get_uniforms()
            self._fragment_shader = gloo.FragmentShader(generated_code, version=self._glsl_version)

        compile_time = time.time() - start_time
        _logger.debug("Recompiled {} in {:.6f}ms".format(node.title, compile_time * 1000))
    ...
    def get_program_copy(self):
        print("Copy wanted! Handle this shit")
        vertex_shader = self.compile_vertex_shader(new_instance=True)
        fragment_shader = self.compile_fragment_shader(new_instance=True)
    ...
    def _compile(self, node: 'Node' = None) -> typing.Tuple[gloo.VertexShader, gloo.FragmentShader, gloo.Program]:
        """
    ...
    def recompile(self, node: 'Node' = None):
        ...
        """
        self._vertex_shader, self._fragment_shader, self._program = self._compile(node)

    def _set_program(self):
        self._vertex_shader, self._fragment_shader, self._program = self._compile()
        self.set_defaults()
