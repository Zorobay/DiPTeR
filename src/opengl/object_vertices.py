...
def get_2d_plane():
    """
    Returns a tuple of an array of 4 vertices corresponding to a plane as well as an array with vertex indices for triangles.
    """
    V = np.zeros(4, [(IN_VERTEX_POS_NAME, np.float32, 3)])
    V[IN_VERTEX_POS_NAME] = np.array([(-1, -1, 0), (1, -1, 0), (1, 1, 0), (-1, 1, 0)], dtype=np.float32)
...
def get_3d_cube():
    """
    Returns a tuple of an array of 8 vertices corresponding to a cube as well as an array with vertex indices for triangles.
    """
